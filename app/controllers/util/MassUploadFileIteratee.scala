...
trait MassUploadFileIteratee {
  ...
  private def bufferedUploadIteratee(upload: GroupedFileUpload, position: Long, bufferSize: Int): Iteratee[Array[Byte], Result] = {
    if (position > upload.uploadedSize) {
      ...
      val it = uploadIteratee(upload.id, position)
      Traversable.takeUpTo[Array[Byte]](bufferSize).transform(it)
    }
  }

  private def uploadIteratee(uploadId: Long, initialPosition: Long): Iteratee[Array[Byte], Result] = {
    Iteratee.foldM(initialPosition) { (position: Long, bytes: Array[Byte]) =>
      for {
        _ <- groupedFileUploadBackend.writeBytes(uploadId, position, bytes)
